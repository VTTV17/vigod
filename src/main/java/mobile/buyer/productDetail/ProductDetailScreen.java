...
public class ProductDetailScreen extends ProductDetailElement {
    WebDriver driver;
    WebDriverWait wait;
    Logger logger = LogManager.getLogger(ProductDetailScreen.class);
    ...
    void checkProductName(String barcode, String language) {
        ...
        String adrProductName = commonAndroid.getElement(loc_lblProductName).getText();
    ...
    void checkFlashSaleShouldBeShown(String brName) {
        ...
        try {
            commonAndroid.getElement(loc_lblFlashSaleBadge);
    ...
    void checkDiscountCampaignShouldBeShown(String brName) {
        ...
        try {
            commonAndroid.getElement(loc_lblDiscountCampaignBadge);
        } catch (NoSuchElementException ex) {
    ...
    void checkWholesaleProductShouldBeShown(String brName) {
        ...
        boolean check = true;
        try {
            commonAndroid.getElement(loc_lblWholesaleProductBadge);
        } catch (NoSuchElementException ex) {
            check = false;
        }
    ...
    void checkVariationName(String language) {
        ...
        variationNameListDB.forEach(variationName -> assertCustomize.assertTrue(commonAndroid.isTextShown(variationName), "Can not find variation name: %s".formatted(variationName)));
    ...
    void checkFilterAndSearchBranchIsShown(String... variationName) {
        ...
        try {
            commonAndroid.getElement(SEARCH_BRANCH_ICON).click();
        } catch (NoSuchElementException ex) {
        ...
        try {
            ...
            commonAndroid.getElement(FILTER_BRANCH_ICON);
        } catch (NoSuchElementException ex) {
            checkFilter = false;
        }
    ...
    void checkBranchNameAndStock(String brName, boolean brStatus, int brStock, String... variationName) {
        String varName = variationName.length > 0 ? ((!variationName[0].isEmpty()) ? "[Variation: %s]".formatted(variationName[0]) : "") : "";

        // check branch information
        // check branch name
        assertCustomize.assertTrue(brInfo.getBranchName().contains(brName) && brStatus && (brStock > 0), "[Failed][Branch name: %s] Branch in-stock but is not shown.".formatted(brName));

        if (!productInfo.isHideStock() & brStatus) {
            // check branch stock
            int adrBranchStock = Integer.parseInt(commonAndroid.getText(loc_lblBranch(brName)).split(" - ")[1].replaceAll("\\D+", ""));
            assertCustomize.assertEquals(adrBranchStock, brStock, "[Failed]%s[Branch name: %s] Stock quantity should be %s, but found %s".formatted(varName, brName, brStock, adrBranchStock));
        } else logger.info("Setting hide stock.");
        logger.info("%s Check branch '%s'".formatted(varName, brName));
    }
    ...
    void checkProductDescription(String barcode, String language) {
        ...
        if (dbDescription.length() > 1) {
            commonAndroid.scrollToEndOfScreen();
            String adrDescription = commonAndroid.getText( loc_lblDescriptionContent);
    ...
    void checkSoldOutMark(String... variationName) {
        String varName = variationName.length > 0 ? ((!variationName[0].isEmpty()) ? "[Variation: %s]".formatted(variationName[0]) : "") : "";
        boolean isSoldOut = commonAndroid.getElement(loc_lblSoldOutMark) != null;
    ...
    void checkBuyNowAddToCartAndContactNowBtn(String... variationName) {
        ...
        if (!(new Preferences(loginInformation).isEnabledListingProduct() && productInfo.isEnabledListing())) {
            ...
            boolean checkBuyNow = commonAndroid.getElement(BUY_NOW_BTN) != null;
            ...
            boolean checkAddToCart = commonAndroid.getElement(ADD_TO_CART_ICON) != null;
            ...
            boolean isContactNow = commonAndroid.getElement(CONTACT_NOW_BTN) != null;
    ...
    void checkVariationPrice(int varIndex, long listingPrice, long sellingPrice, long flashSalePrice, int wholesaleProductStock, long wholesaleProductPrice, String brName) {
        ...
        if (!(isEnableListingProduct && productInfo.isEnabledListing())) {
            ...
            if (!Objects.equals(sellingPrice, listingPrice)) {
                String adrListingPrice = commonAndroid.getElement(rsId_lblListingPrice).getText().replaceAll("\\D+", "");
            ...
            commonAndroid.getElement(ADD_TO_CART_ICON).click();
            ...
            commonAndroid.tapByCoordinatesInPercent(0.5, 0.5);
        }
    }
    ...
    void checkVariationInformation(int varIndex, long listingPrice, long sellingPrice, long flashSalePrice, int wholesaleProductStock, long wholesaleProductPrice, List<Integer> branchStock, String language, String... variationName) {
        ...
        if (numberOfDisplayBranches > 0) {
            ...
            for (String brName : brInfo.getActiveBranches()) {
                // switch branch
                commonAndroid.click(rsId_sctBranch, loc_lblBranch(brName));
        ...
        for (int varIndex = 0; varIndex < variationModelList.size(); varIndex++) {
            ...
            if (productInfo.getVariationStatus().get(varIndex).equals("ACTIVE")) {
                ...
                if (productInfo.isHasModel())
                    Arrays.stream(variationValue.split("\\|")).forEachOrdered(var -> commonAndroid.moveAndGetOverlappedElementByText(var, ITEM_DETAIL_FOOTER).click());
    ...
    public BuyerShopCartPage buyNowProduct(int quantity) {
        commonAndroid.click(BUY_NOW_BTN);
        if (!commonAndroid.getText(BUY_NOW_POPUP_QUANTITY_TEXT_BOX).equals(String.valueOf(quantity))) {
            commonAndroid.sendKeys(BUY_NOW_POPUP_QUANTITY_TEXT_BOX, String.valueOf(quantity));
        }
        commonAndroid.click(BUY_NOW_POPUP_BUY_BTN);
        return new BuyerShopCartPage(driver).waitLoadingDisapear();
    }

    public ProductDetailScreen addToCart(int quantity) {
        commonAndroid.click(ADD_TO_CART_ICON);
        if (!commonAndroid.getText(ADD_TO_CART_POPUP_QUANTITY_TEXT_BOX).equals(String.valueOf(quantity))) {
            commonAndroid.sendKeys(ADD_TO_CART_POPUP_QUANTITY_TEXT_BOX, String.valueOf(quantity));
        }
        commonAndroid.click(ADD_TO_CART_POPUP_ADD_BTN);
        logger.info("Add product to cart");
        return this;
    }

    public BuyerShopCartPage tapOnShoppingCart() {
        commonAndroid.click(CART_ICON);
    ...
    public boolean isReviewTabDisplayed() {
    	commonAndroid.getElement(PRODUCT_DESCRIPTION_TAB);
        boolean isDisplayed = commonAndroid.isShown(PRODUCT_REVIEW_TAB);
    ...
    public String[] getReview() {
    	...
    	commonAndroid.getElement(REVIEWCONTENT);
    	
    	//Temporary
    	if (commonAndroid.isShown(EMPTYREVIEW)) {
    		return null;
    	}
    	
    	String title = commonAndroid.getText(PRODUCT_REVIEW_TITLE);
    	String description = commonAndroid.getText(PRODUCT_REVIEW_DESCRIPTION);
