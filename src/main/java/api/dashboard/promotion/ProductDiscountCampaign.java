...
public class ProductDiscountCampaign {
    ...
    public DiscountCampaignInfo getDiscountCampaignInfo(ProductInfo productInfo, List<Integer> listSegmentOfCustomer) {
        ...
        List<Integer> inProgressList = api.get(DISCOUNT_CAMPAIGN_SCHEDULE_LIST_PATH.formatted(loginInfo.getStoreID()), loginInfo.getAccessToken())
                .jsonPath()
                .getList("id");
        ...
        // get list schedule discount campaign
        List<Integer> scheduleList = api.get(DISCOUNT_CAMPAIGN_IN_PROGRESS_LIST_PATH.formatted(loginInfo.getStoreID()), loginInfo.getAccessToken())
                .jsonPath()
                .getList("id");
        ...
        if (discountCampaignPrice == null || discountCampaignPrice.isEmpty())
            discountCampaignPrice = new ArrayList<>(productInfo.getProductSellingPrice());
        ...
        if (discountCampaignStatus == null || discountCampaignStatus.keySet().isEmpty() || discountCampaignList.isEmpty()) {
            ...
            brInfo.getBranchName().forEach(brName -> discountCampaignStatus.put(brName,
                    productInfo.getVariationModelList()
                            .stream()
                            .map(barcode -> "EXPIRED")
                            .toList()));
        }
        ...
        discountCampaignList.forEach(campaignID -> getDiscountCampaignInformation(campaignID, productInfo, listSegmentOfCustomer));
