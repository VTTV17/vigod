...
public class DiscountCode {
    public void createDiscountCode(String account, String password) {
        ...
        for (int rewardId = 0; rewardId < 2; rewardId++) {
            // coupon name
            String name1 = rewardId == 0 ? "notReward_" : "isReward_";
            ...
            for (int limitToOne = 0; limitToOne < 2; limitToOne++) {
                ...
                for (int limitUsage = 0; limitUsage < 2; limitUsage++) {
                    ...
                    name3 += couponLimitedUsage ? "notLimitUsage_" : "isLimitUsage %s times_".formatted(couponTotal);
                    // coupon type
                    // 0: percentage
                    // 1: fixed amount
                    // 2: free shipping
                    for (int couponType = 0; couponType < 3; couponType++) {
                        ...
                        name4 += couponType == 0 ? "Percentage %s_".formatted(couponValue) : couponType == 1 ? "FixAmount %s_".formatted(couponValue) : "FreeShipping%s_".formatted(couponValue);
