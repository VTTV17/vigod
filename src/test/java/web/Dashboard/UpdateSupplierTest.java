...
public class UpdateSupplierTest extends BaseTest {
    ...
    void UP_SUPPLIER_01_ViewSupplierDetail() {
        ...
        new SupplierPage(driver).getLoginInformation(loginInformation).setLanguage(language).checkSupplierInformation();
    }

    @Test
    void UP_SUPPLIER_02_UpdateVNSupplier() throws Exception {
        testCaseId = "UP_SUPPLIER_02";
        new SupplierPage(driver).getLoginInformation(loginInformation).setLanguage(language).updateSupplier(true);
    }

    @Test
    void UP_SUPPLIER_03_UpdateNonVNSupplier() throws Exception {
        testCaseId = "UP_SUPPLIER_03";
        new SupplierPage(driver).getLoginInformation(loginInformation).setLanguage(language).updateSupplier(false);
    }
    ...
    void UP_SUPPLIER_07_CheckOrderHistory() throws Exception {
        ...
        new SupplierPage(driver).getLoginInformation(loginInformation).setLanguage(language).checkOrderHistory();
    }

    @Test
    void UP_SUPPLIER_08_DeleteSupplier() throws Exception {
        testCaseId = "UP_SUPPLIER_08";
        new SupplierPage(driver).getLoginInformation(loginInformation).setLanguage(language).deleteSupplier();
    }
}
